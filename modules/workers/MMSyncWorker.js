...
function winCreateHiddenWindowForMessageLoop() {
	...
	var windowProc = function(hwnd, uMsg, wParam, lParam) {
		...
		if (cutils.jscEqual(uMsg, ostypes.CONST.WM_INPUT)) {
			// console.info('lParam:', lParam, lParam.toString());
			var hrawinput = ostypes.TYPE.HRAWINPUT(lParam); // ctypes.cast(lParam, ostypes.TYPE.HRAWINPUT) doesnt work here as the Message.lParam is not really ostypes.TYPE.LPARAM it gets unwrapped so its primiated js type, its just a number. thats why i can just wrap it with a ostypes.TYPE.HRAWINPUT
			var rez_getRawInputData = ostypes.API('GetRawInputData')(hrawinput, ostypes.CONST.RID_INPUT, OSStuff.getRawInputDataBuffer.address(), OSStuff.rawInputDataBufferSize.address(), ostypes.TYPE.RAWINPUTHEADER.size);
	...
	// rawinput stuff from tutorial: http://www.toymaker.info/Games/html/raw_input.html#tables
				...
				*/
				...
				if (rez_registerDevices == false) {
					...
					throw new Error('failed to register devices');
				}
